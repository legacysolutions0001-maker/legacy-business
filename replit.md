...
A production-grade commercial ERP system for managing multiple client companies. Includes invoicing, inventory, HR, payroll, CRM, reporting, appointments, expenses, projects, day book, e-way bills, cash & bank, messaging, ledger, and a super admin portal for managing companies, licenses, and subscriptions.
...
- Password: read from `SUPERADMIN_PASSWORD` secret
...
- Project ID: `legacy-business-erp`
- Auth Domain: `legacy-business-erp.firebaseapp.com`
- Storage Bucket: `legacy-business-erp.firebasestorage.app`
- Messaging Sender ID: `41637512726`
- App ID: `1:41637512726:web:f3fbde31f97b35f4525737`
- API Key: stored in `FIREBASE_API_KEY` secret
- Service Account: stored in `FIREBASE_SERVICE_ACCOUNT_JSON` secret
...
- Firebase used ONLY for auth, company registration, licensing — NOT for ERP business data
- All business data (customers, inventory, invoices, etc.) stored in local PostgreSQL
...
- Secrets: `GITHUB_PAT`, `SUPER_ADMIN_PASSWORD`, `FIREBASE_API_KEY`, `FIREBASE_SERVICE_ACCOUNT_JSON`, `SESSION_SECRET`
- GitHub repo: https://github.com/legacysolutions0001-maker/legacy-business
- GitHub branch: main
- Super Admin username: `bhullar01`
- After EVERY completed milestone: git add → commit → push → verify

## Gotchas

- `DATABASE_URL` is runtime-managed by Replit — do not set it manually via `setEnvVars`
- Generated Zod schemas live in `lib/api-zod/src/generated/api.ts` — run codegen after any OpenAPI spec change
- API routes import Zod schemas from `@workspace/api-zod`; never guess schema names — run codegen and grep exports
