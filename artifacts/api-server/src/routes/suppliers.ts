...
router.get("/suppliers", requireResolvedCompany, async (req, res) => {
...
router.post("/suppliers", requireResolvedCompany, async (req, res) => {
  const companyId = resolveCompanyId(req);
  if (!companyId) { res.status(403).json({ error: "Company required" }); return; }
...
router.get("/suppliers/:id", requireResolvedCompany, async (req, res) => {
  const companyId = resolveCompanyId(req);
...
router.patch("/suppliers/:id", requireResolvedCompany, async (req, res) => {
  const companyId = resolveCompanyId(req);
...
router.delete("/suppliers/:id", requireResolvedCompany, async (req, res) => {
  const companyId = resolveCompanyId(req);
