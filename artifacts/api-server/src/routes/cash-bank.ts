...
router.get("/cash-bank/summary", requireAuth, async (req, res) => {
...
router.get("/cash-bank", requireAuth, async (req, res) => {
  const companyId = req.auth!.companyId;
  if (!companyId) { res.status(403).json({ error: "Company required" }); return; }
...
router.post("/cash-bank", requireAuth, async (req, res) => {
  const companyId = req.auth!.companyId;
  if (!companyId) { res.status(403).json({ error: "Company required" }); return; }
...
router.patch("/cash-bank/:id", requireAuth, async (req, res) => {
  const companyId = req.auth!.companyId;
...
router.delete("/cash-bank/:id", requireAuth, async (req, res) => {
  const companyId = req.auth!.companyId;
