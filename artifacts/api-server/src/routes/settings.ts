...
import { requireResolvedCompany, requireSuperAdmin, resolveCompanyId } from "../middlewares/auth";
...
router.get("/settings", requireResolvedCompany, async (req, res) => {
...
router.patch("/settings/company", requireResolvedCompany, async (req, res) => {
  const companyId = resolveCompanyId(req);
  if (!companyId) { res.status(403).json({ error: "Company required" }); return; }
...
router.patch("/settings/invoice", requireResolvedCompany, async (req, res) => {
  const companyId = resolveCompanyId(req);
  if (!companyId) { res.status(403).json({ error: "Company required" }); return; }
