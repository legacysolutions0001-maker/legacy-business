...
router.get("/notifications", requireResolvedCompany, async (req, res) => {
...
router.get("/notifications/unread-count", requireResolvedCompany, async (req, res) => {
...
router.post("/notifications/read-all", requireResolvedCompany, async (req, res) => {
  const { companyId } = req.auth!;
...
router.patch("/notifications/:id/read", requireResolvedCompany, async (req, res) => {
  await db.update(notificationsTable).set({ isRead: true }).where(eq(notificationsTable.id, parseInt(req.params.id as string)));
  res.json({ success: true });
});

router.post("/notifications", requireResolvedCompany, async (req, res) => {
  const { companyId } = req.auth!;
