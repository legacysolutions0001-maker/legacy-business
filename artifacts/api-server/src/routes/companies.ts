...
router.get("/companies/:id/features", requireAuth, async (req, res) => {
...
router.get("/companies/me/info", requireAuth, async (req, res) => {
...
router.patch("/companies/me/info", requireAuth, async (req, res) => {
  if (!req.auth?.companyId) { res.status(403).json({ error: "Company required" }); return; }
