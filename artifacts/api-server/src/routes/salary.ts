...
router.get("/salary", requireResolvedCompany, async (req, res) => {
...
router.post("/salary", requireResolvedCompany, async (req, res) => {
  const companyId = resolveCompanyId(req);
  if (!companyId) { res.status(403).json({ error: "Company required" }); return; }
...
router.patch("/salary/:id", requireResolvedCompany, async (req, res) => {
  const companyId = resolveCompanyId(req);
  const update: any = {};
...
router.delete("/salary/:id", requireResolvedCompany, async (req, res) => {
  const companyId = resolveCompanyId(req);
  if (!companyId) { res.status(403).json({ error: "Company required" }); return; }
...
router.get("/salary/summary", requireResolvedCompany, async (req, res) => {
  const companyId = resolveCompanyId(req);
  if (!companyId) { res.status(403).json({ error: "Company required" }); return; }
  const { month, year } = req.query as { month?: string; year?: string };
