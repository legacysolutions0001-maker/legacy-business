...
router.get("/stock-transactions", requireResolvedCompany, async (req, res) => {
...
router.get("/stock-transactions/product/:productId", requireResolvedCompany, async (req, res) => {
  const companyId = resolveCompanyId(req);
  if (!companyId) { res.status(403).json({ error: "Company required" }); return; }
