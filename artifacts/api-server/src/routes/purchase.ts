...
router.get("/purchase-orders/summary", requireAuth, async (req, res) => {
...
router.patch("/purchase-orders/:id", requireAuth, async (req, res) => {
  const companyId = req.auth!.companyId;
  const [row] = await db.update(purchaseOrdersTable).set(req.body).where(and(eq(purchaseOrdersTable.id, parseInt(req.params.id as string)), eq(purchaseOrdersTable.companyId, companyId!))).returning();
  if (!row) { res.status(404).json({ error: "Not found" }); return; }
...
router.delete("/purchase-orders/:id", requireAuth, async (req, res) => {
  const companyId = req.auth!.companyId;
  await db.delete(purchaseOrdersTable).where(and(eq(purchaseOrdersTable.id, parseInt(req.params.id as string)), eq(purchaseOrdersTable.companyId, companyId!)));
  res.sendStatus(204);
});
