...
router.get("/stock-batches", requireAuth, async (req, res) => {
  const companyId = req.auth!.companyId;
  if (!companyId) { res.status(403).json({ error: "Company required" }); return; }
...
router.patch("/stock-batches/:id", requireAuth, async (req, res) => {
  const companyId = req.auth!.companyId;
...
router.get("/product-variants/:productId", requireAuth, async (req, res) => {
  const companyId = req.auth!.companyId;
  const variants = await db.select().from(productVariantsTable)
...
router.post("/product-variants", requireAuth, async (req, res) => {
  const companyId = req.auth!.companyId;
  if (!companyId) { res.status(403).json({ error: "Company required" }); return; }
  const d = req.body;
...
router.patch("/product-variants/:id", requireAuth, async (req, res) => {
  const companyId = req.auth!.companyId;
  const update: any = { ...req.body };
  if (update.sellingPrice != null) update.sellingPrice = String(update.sellingPrice);
  if (update.purchasePrice != null) update.purchasePrice = String(update.purchasePrice);
