...
export default function SuperSettings() {
  ...
  const changePass = useMutation({
    mutationFn:(d:any)=>apiFetch("/auth/change-password",{method:"POST",body:JSON.stringify(d)}).then(r=>{if(!r.ok)throw new Error("Failed");return r.json();}),
    ...
    <div className="space-y-5 max-w-2xl">
      ...
      <Card className="bg-slate-900/50 border-slate-800">
        ...
        <CardContent className="space-y-2 text-sm">
          ...
          <Separator className="bg-slate-800"/>
          <div className="flex justify-between"><span className="text-slate-400">Auth Mode</span><span className="text-white">JWT Cookie (httpOnly)</span></div>
        </CardContent>
      </Card>
    </div>
  );
